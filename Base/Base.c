...
ISR(TIMER0,0)
  {
    ...
    if(sec_time_counter_helper == 1000)
      ...
      { // Ensure that time spent in the ISR takes the same amount of time on both execution paths
        __asm
        nop
        nop
        nop
...
// Wait for seconds
void
delay_sec(unsigned int sec)
{
  if (sec == 0)
    return;   // Return if delaytime is zero
...
void
reset_timeout(timer_id_type id, timer_type type)
{
...
unsigned int
get_time_elapsed(timer_id_type id, timer_type type)
{
...
{
return ((unsigned int) ((unsigned int) 0xffff - timer_start_times[id]))
  + counter;
}
...
bool
timeout_occured(timer_id_type id, timer_type type, unsigned int timeout_limit)
{
